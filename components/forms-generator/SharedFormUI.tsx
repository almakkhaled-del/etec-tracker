    ...
    <div style={{ marginBottom: 14 }}>
      ...
      <textarea value={value} onChange={e => onChange(e.target.value)} placeholder={placeholder} rows={rows}
        style={{
          width: '100%', padding: '10px 12px', border: '1.5px solid rgba(11,31,58,0.12)',
          borderRadius: 9, fontSize: 13, fontFamily: 'IBM Plex Sans Arabic, sans-serif',
          boxSizing: 'border-box', background: '#FAFAF7', color: NAVY, direction: 'rtl', resize: 'vertical'
        }}
      />
    </div>
  )
...
export function ItemCard({ children, onRemove, canRemove }: {
  ...
  return (
    <div style={{ border: '1.5px solid rgba(11,31,58,0.1)', borderRadius: 12, padding: 16, marginBottom: 14, position: 'relative', background: '#FAFAF7' }}>
      {canRemove && (
  ...
  return (
    <main style={{ padding: '24px 28px', maxWidth: 640, margin: '0 auto' }}>
      <div style={{ marginBottom: 16 }}>
        ...
        <p style={{ fontSize: 12, color: '#8A8270', margin: 0, fontFamily: 'IBM Plex Sans Arabic, sans-serif' }}>{subtitle}</p>
      </div>
      {children}
    </main>
  )
