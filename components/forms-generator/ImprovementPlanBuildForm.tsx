...
interface ImprovementPlanBuildFormProps {
  schoolPrincipalName: string;
  onGenerated?: (fileName: string) => void;
}

export default function ImprovementPlanBuildForm({
  schoolPrincipalName,
  onGenerated,
}: ImprovementPlanBuildFormProps) {
  ...
  async function handleGenerate() {
    ...
    try {
      const res = await fetch(
        "/templates/improvement-plan-build-template.docx"
      );
      ...
      doc.render({
        ...
        learning_outcomes_level: learningOutcomesLevel,
        items: filledItems,
        recommendations,
      ...
      const fileName = `استمارة 1 - بناء خطة التحسين - ${schoolName}.docx`;
  ...
  return (
    <div dir="rtl" className="max-w-3xl mx-auto space-y-6 p-4">
      <h2 className="text-xl font-bold">
        استمارة (1): بناء خطة التحسين في مجالات الممارسات الإشرافية
      </h2>
      ...
      <section className="space-y-3">
        ...
        <div className="grid grid-cols-2 gap-3">
          <input
            className="border rounded-lg p-2"
            placeholder="مستوى الأداء العام (آخر تقرير)"
            value={overallPerformanceLevel}
            onChange={(e) => setOverallPerformanceLevel(e.target.value)}
          />
          <input
            className="border rounded-lg p-2"
            placeholder="مستوى نواتج التعلم (آخر تقرير)"
            value={learningOutcomesLevel}
            onChange={(e) => setLearningOutcomesLevel(e.target.value)}
          />
        </div>
      </section>
      ...
      <section className="space-y-3">
        <h3 className="font-semibold">
          ثانياً: إجراءات خطة التحسين في مجالات الممارسات الإشرافية
        </h3>
        {items.map((item, i) => (
          <div key={i} className="border rounded-lg p-3 space-y-2 relative">
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <input
                className="border rounded-lg p-2"
                placeholder="العنصر / المكون المراد تحسينه"
                value={item.element}
            ...
            <textarea
              ...
              rows={2}
              value={item.need}
              onChange={(e) => updateItem(i, "need", e.target.value)}
            />
            <textarea
              className="w-full border rounded-lg p-2"
              placeholder="إجراءات التحسين"
              rows={2}
              value={item.actions}
              onChange={(e) => updateItem(i, "actions", e.target.value)}
            />
            <textarea
              className="w-full border rounded-lg p-2"
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <input
                className="border rounded-lg p-2"
                placeholder="التنفيذ والمسؤولية"
                value={item.responsibility}
                onChange={(e) =>
                  updateItem(i, "responsibility", e.target.value)
                }
              />
            </div>
          </div>
