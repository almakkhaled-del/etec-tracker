// دالة بناء ملف الخطة التشغيلية (docx) — استُخرجت من
// app/forms/operational-plan/page.tsx إلى هنا حتى تُستخدم من أكثر من مكان
// (صفحة الخطة التشغيلية القائمة وحدها، وصفحة "بناء الخطط" الموحّدة الجديدة)
// بدون تكرار ~450 سطر. السلوك مطابق تماماً للأصل — نقل بدون أي تعديل منطقي.
...
export const STAGE_GOALS: Record<string, string[]> = {
  'ابتدائية': [
...
export interface OperationalPlanInfo {
  schoolName: string
...
export async function buildOperationalPlanDocx(aiData: any, info: OperationalPlanInfo) {
  const [
