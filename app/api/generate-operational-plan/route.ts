...
const GEMINI_URL = 'https://generativelanguage.googleapis.com/v1beta/models/gemini-3.1-flash-lite-latest:generateContent'

const PROMPT = `أنت خبير في تحليل تقارير التقويم المدرسي السعودي وفق إطار إتقان.

اقرأ تقرير التقويم الخارجي المرفق واستخرج:

1. بيانات المدرسة من التقرير: اسم المدرسة، إدارة التعليم، مكتب التعليم، المرحلة الدراسية (ابتدائية/متوسطة/ثانوية)
    ...
    sections: [{
      ...
      children: [
        ...
        tbl([
          ...
          new TableRow({ children: [ lC('مكتب التعليم', 2200), dC(info.district, 3500), lC('المرحلة', 2500), dC(info.stage, PW - 8200) ] }),
        ]),
        sp(200),
