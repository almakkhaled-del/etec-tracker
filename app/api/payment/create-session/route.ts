...
import { createCheckoutSession, isGatewayConfigured, ANNUAL_PLAN } from '@/lib/payment'

// ينشئ جلسة دفع (جيديا HPP) للمدرسة صاحبة الجلسة الحالية — يُستدعى من صفحة /subscribe.
...
export async function POST(req: NextRequest) {
  try {
    ...
    // بيانات التواصل تُمرَّر لجيديا لتعبئة صفحة الدفع مسبقاً (اختياري)
    const { data: school } = await supabase
      .from('schools').select('email, phone').eq('id', schoolUser.school_id).single()

    const session = await createCheckoutSession(schoolUser.school_id, {
      email: school?.email || user.email || undefined,
      phone: school?.phone || undefined,
    })
    if (!session) return NextResponse.json({ configured: false, plan: ANNUAL_PLAN })

    return NextResponse.json({ configured: true, url: session.url, reference: session.reference })
  } catch (e: any) {
