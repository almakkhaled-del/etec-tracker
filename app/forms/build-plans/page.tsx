...
// buildOperationalPlanDocx مستوردة من ملف مشترك (lib/operationalPlanDocx.ts)
// بدل تكرار كودها هنا — نفس الدالة المستخدمة بصفحة /forms/operational-plan
// القائمة لوحدها (بقيت شغالة كما هي، بدون ربطها بالقائمة الجانبية بعد الآن).
...
type DocStatus = 'idle' | 'generating' | 'done' | 'unavailable'
...
export default function BuildPlansPage() {
  ...
  const [operationalAiData, setOperationalAiData] = useState<any>(null)
  ...
  async function handleAnalyze() {
    if (!file) return
    setStep('analyzing'); setProgress('جاري قراءة التقرير...'); setError(''); setOperationalError('')
    try {
      const base64 = await readFileAsBase64(file)
      setProgress('يحلل النظام التقرير ويبني الخطط الأربع...')

      const domainGroups = ['admin', 'teaching', 'outcomes', 'environment'] as const
      const STAGGER_MS = 1500
      const delay = (ms: number) => new Promise(r => setTimeout(r, ms))

      // نفس تفريق التوقيت المستخدم أصلاً لتفادي rate limit عند Gemini — أضفنا
      // طلب الخطة التشغيلية كطلب خامس مفرّق بنفس المنطق (بعد آخر مجموعة مؤشرات).
      ...
      const operationalPromise = principalName
        ? delay(domainGroups.length * STAGGER_MS).then(() =>
            fetch('/api/generate-operational-plan', {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              ...
              if (!r.ok) {
                ...
                throw new Error(err.error || 'فشل توليد الخطة التشغيلية')
              }
              return r.json()
            })
          ).catch((e: any) => ({ __error: e.message }))
        : Promise.resolve({ __error: 'اسم مدير المدرسة غير مسجّل — أضفه من إعدادات المدرسة لتوليد الخطة التشغيلية' })

      const [info, operationalResult, ...groupSettled] = await Promise.all([infoPromise, operationalPromise, ...groupPromises])
      ...
      if (operationalResult && (operationalResult as any).__error) {
        setOperationalError((operationalResult as any).__error)
        setDocStatus(s => ({ ...s, doc4: 'unavailable' }))
      } else {
        setOperationalAiData(operationalResult)
      }
  ...
  async function downloadDoc1() {
    ...
    try {
      const { Document, Packer, Table, TableRow, WidthType, hCell, gCell, dCell, title, section, gap, saveAs } = await buildDocxHelpers()
      const d = result
      const landscapeProps = { page: { size: { width: 16838, height: 11906 }, margin: { top: 720, bottom: 720, left: 720, right: 720 } } }
      ...
      const doc = new Document({ styles: { default: { document: { run: { font: 'Sakkal Majalla', size: 24 } } } }, sections: [{ properties: landscapeProps, children: [
        ...
        new Table({ visuallyRightToLeft: true,
          ...
          rows: [
            ...
            new TableRow({ children: [
              dCell('', 3592), dCell('', 2329), dCell('', 3125), dCell('', 3125), dCell('', 3217),
            ]}),
          ]
        })
      ]}]})
  ...
  async function downloadDoc3() {
    ...
    try {
      const { Document, Packer, Table, TableRow, WidthType, hCell, gCell, dCell, bulletCell, title, section, gap, saveAs } = await buildDocxHelpers()
      const d = result
  ...
  async function downloadDoc4() {
    if (!operationalAiData || !result) return
    setDocStatus(s => ({ ...s, doc4: 'generating' }))
    try {
      const si = operationalAiData.school_info || {}
      const info = {
        schoolName: si.school_name || result.school_name || 'المدرسة',
        principalName: result.principal_name || school?.principal_name || '',
        region: si.region || '',
        district: si.district || '',
        stage: si.stage || 'ابتدائية'
      }
      await buildOperationalPlanDocx(operationalAiData, info)
      setDocStatus(s => ({ ...s, doc4: 'done' }))
    } catch (e: any) {
      alert('خطأ في توليد الملف: ' + e.message)
      setDocStatus(s => ({ ...s, doc4: 'idle' }))
    }
  }

  const DOCS = [
    { key: 'doc1', num: '1', icon: '📋', title: 'خطة التحسين', desc: 'استمارة رسمية مملوءة بالمؤشرات الضعيفة وإجراءات التحسين', fn: downloadDoc1, available: !!result },
    { key: 'doc2', num: '2', icon: '📝', title: 'تنفيذ التحسين', desc: 'جدول متابعة الإجراءات المنفذة جاهز للتوثيق', fn: downloadDoc2, available: !!result },
    { key: 'doc3', num: '3', icon: '📊', title: 'واقع المدرسة', desc: 'تحليل SWOT والأولويات العاجلة ومؤشرات الأداء', fn: downloadDoc3, available: !!result },
    { key: 'doc4', num: '4', icon: '🗂️', title: 'الخطة التشغيلية ومتابعتها', desc: 'خطة تشغيلية كاملة وفق 10 أهداف مبنية على واقع المدرسة', fn: downloadDoc4, available: !!operationalAiData },
  ]

  // قفل الباقة المجانية: بناء الخطط متاح بالاشتراك المدفوع فقط.
    ...
    <div style={{ minHeight: '100vh', background: CREAM, fontFamily: "'Tajawal', sans-serif", direction: 'rtl' }}>
      ...
      <div style={{ display: 'flex', minHeight: '100vh' }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <main className="page-main" style={{ padding: '24px 28px', maxWidth: 700, margin: '0 auto' }}>
            ...
            {(step === 'upload' || step === 'error') && (
              <div style={{ background: '#fff', borderRadius: 18, border: '1px solid rgba(10,59,88,0.07)', padding: '1.5rem 1.8rem', boxShadow: '0 4px 16px rgba(10,59,88,0.06)' }}>
                <div style={{ background: '#F0F7FF', border: '1px solid #BFDBFE', borderRadius: 12, padding: '14px 18px', marginBottom: 20 }}>
                  <p style={{ fontSize: 13, fontWeight: 700, color: '#1E40AF', margin: '0 0 8px' }}>هذه الصفحة خاصة في بناء</p>
                  <ul style={{ margin: '0 0 8px', padding: '0 18px', fontFamily: 'IBM Plex Sans Arabic, sans-serif', fontSize: 13, color: '#1E3A8A', lineHeight: 2 }}>
                    <li>الخطة التشغيلية ومتابعتها</li>
                    <li>خطة التحسين</li>
                    <li>تنفيذ التحسين</li>
                    <li>واقع المدرسة</li>
                  </ul>
                  <p className="body-font" style={{ fontSize: 13, color: '#1E3A8A', margin: 0, lineHeight: 1.8 }}>
                    فضلاً قم برفع آخر تقرير للتقويم الخارجي لتحليل النتائج والبناء وفق ذلك.
                  </p>
                </div>
                ...
                {!school?.principal_name && (
                  <div style={{ background: '#FFFBEB', border: '1px solid #FCD34D', borderRadius: 10, padding: '10px 16px', marginBottom: 16 }}>
                    <p className="body-font" style={{ fontSize: 12.5, color: '#92400E', margin: 0, lineHeight: 1.8 }}>
                      ⚠️ اسم مدير/ة المدرسة غير مسجّل بإعدادات المدرسة — بدونه لن تتمكن من توليد ملف الخطة التشغيلية (الملفات الثلاثة الأخرى تعمل بشكل طبيعي).
                    </p>
                  </div>
                )}
            ...
            {step === 'analyzing' && (
              <div style={{ background: '#fff', borderRadius: 18, border: '1px solid rgba(10,59,88,0.07)', padding: '3rem 1.8rem', boxShadow: '0 4px 16px rgba(10,59,88,0.06)', textAlign: 'center' }}>
                ...
                <p className="body-font" style={{ fontSize: 13, color: '#7A8896', margin: '0 0 20px' }}>يعالج النظام التقرير ويبني الخطط الأربع</p>
                <div style={{ background: 'rgba(10,59,88,0.04)', borderRadius: 10, padding: '12px 16px' }}>
                  <p className="body-font" style={{ fontSize: 12, color: '#7A8896', margin: 0 }}>قد يستغرق 30-90 ثانية</p>
                </div>
              </div>
            )}
            ...
            {step === 'ready' && result && (
              <>
                ...
                {operationalError && (
                  <div style={{ background: '#FFFBEB', border: '1.5px solid #FCD34D', borderRadius: 10, padding: '12px 16px', marginBottom: 16 }}>
                    <p className="body-font" style={{ fontSize: 12.5, color: '#92400E', margin: 0, lineHeight: 1.8 }}>⚠️ الخطة التشغيلية: {operationalError}</p>
                  </div>
                )}
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginBottom: 16 }}>
                  {DOCS.map(doc => {
                    ...
                    const disabled = st === 'generating' || !doc.available
                    return (
                      <div key={doc.key} style={{ background: '#fff', borderRadius: 14, border: `1.5px solid ${st === 'done' ? '#86EFAC' : 'rgba(10,59,88,0.08)'}`, padding: '16px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', boxShadow: '0 2px 8px rgba(10,59,88,0.04)' }}>
                        ...
                        <button onClick={doc.fn} disabled={disabled} className="doc-btn"
                          style={{ padding: '10px 20px', fontSize: 13, fontWeight: 700, background: st === 'done' ? '#F0FDF4' : disabled ? '#F3F4F6' : `linear-gradient(135deg, #3E8AB0, ${GOLD})`, color: st === 'done' ? GREEN : disabled ? '#9CA3AF' : NAVY, border: st === 'done' ? `1.5px solid #86EFAC` : 'none', borderRadius: 10, cursor: disabled ? 'not-allowed' : 'pointer', fontFamily: 'Tajawal, sans-serif', whiteSpace: 'nowrap', display: 'flex', alignItems: 'center', gap: 6, boxShadow: (!disabled && st === 'idle') ? '0 3px 10px rgba(31,110,150,0.25)' : 'none', transition: 'all 0.2s' }}>
                          {st === 'generating' && (
                            <svg width="16" height="16" viewBox="0 0 16 16" style={{ animation: 'spin 1s linear infinite' }}>
                              <circle cx="8" cy="8" r="6" fill="none" stroke="#9CA3AF" strokeWidth="2" strokeDasharray="12 26" />
                            </svg>
                          )}
                          {st === 'done' ? 'تم التحميل ✓' : st === 'generating' ? 'جاري التوليد...' : st === 'unavailable' || !doc.available ? 'غير متاح' : '⬇️ تحميل'}
                        </button>
                      </div>
                    )
                  })}
